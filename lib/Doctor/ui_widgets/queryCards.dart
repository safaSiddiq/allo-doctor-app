...
class QueryCards extends StatefulWidget {
  ...
  // QueryCards(this.value);
  @override
  _QueryCardsState createState() => _QueryCardsState();
}

class _QueryCardsState extends State<QueryCards> {
  @override
            ...
            IconButton(
                ...
                onPressed: () {
                  setState(() {
                    ...
                    //     builder: (context) => QueryView(model,query)));
...
//   );
    ...
    Widget queryCard(
      ...
      Future acceptingQuery({
        ...
        final http.Response response = await http.patch(
            //d44adeef-03fe-4af6-8ef0-b6c7db377d2f
            "http://34.71.92.1:3000/doctors/${model.doctorId}/queries",
            headers: {
                                    ...
                                    if (snapShot2.hasData) {
                                      ...
                                      //    value == 0 ?
                                              Container(
                                                  height: 30,
                                                  width: 110,
                                                  child: snapShot2.data.approved == true?acceptButton(context,widget.query):declineButton()
                                                  //   acceptQuery(
                                                  //  context:   context,
                                          ...
                                          //       width: 110,
                                          //       // child:  acceptButton(),
                                          //       child:// snapShot2.data.approved == true?acceptButton(context,query):declineButton() //acceptQuery(snapShot2.data.queryId) :snapShot2.data.approved == true ? acceptButton():declineButton()
                                          //            value == 1
                                          //           ? acceptButton(context,query)
                                          //           : declineButton()
                                          //           )
    ...
    return queryCard(widget.model, widget.query, widget.snapShot,
        widget.patientId, widget.queryId);
  }
}

Widget acceptButton(BuildContext context, Query query) {
  MainModel model;
  return RawMaterialButton(
      onPressed: () {
        Navigator.of(context).push(
            MaterialPageRoute(builder: (context) => QueryView(model, query)));
      },
      child: Container(
        margin: EdgeInsets.symmetric(horizontal: 10, vertical: 2),
        padding: EdgeInsets.symmetric(horizontal: 20, vertical: 8),
